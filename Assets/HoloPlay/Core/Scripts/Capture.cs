...
namespace HoloPlay
{
    ...
    public class Capture : MonoBehaviour
    {
        ...
        [Range(-60, 60)]
        public float verticalAngleOffset = 0;

        [Tooltip("Customize the horizontal angle. Useful if you know the average" +
            "viewer will be looking from the side, instead of head-on")]
        [Range(-60, 60)]
        public float horizontalAngleOffset = 0;

        [Tooltip("Customize the viewcone for this Capture. Allows for partial flattening of 3D content.")]
        [Range(0f, 2f)]
        public float viewConeFactor = 1f;

        // hold on to for setting up cam w/o help from quilt
        ...
        public void RenderView(int view, int numViews, float viewCone)
        {
            HandleOffset(AngleAtView(view, numViews, viewCone) + horizontalAngleOffset, verticalAngleOffset);
        ...
        {
            SetupCam(this.aspect);
        }

        public void SetupCam(float aspect, bool resetOffset = true)
        {
            // set some fields for future uses
            if (aspect < 0)
            {
                if (Calibration.Main != null)
                    Cam.aspect = Calibration.Main.screenW / Calibration.Main.screenH;
                else
                    Cam.ResetAspect();
                aspect = Cam.aspect;
            }
            ...
            if (resetOffset)
                HandleOffset(horizontalAngleOffset, verticalAngleOffset);
        }
        ...
        public void DrawCaptureGizmos()
        {
            SetupCam();
            HandleOffset(horizontalAngleOffset, verticalAngleOffset);
            Gizmos.color = Misc.gizmoColor;
            //get corners
            ...
            // logo
            Gizmos.color = Misc.gizmoLogoColor;
