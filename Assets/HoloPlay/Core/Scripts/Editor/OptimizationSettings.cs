...
namespace HoloPlay.UI
{
    ...
    public class OptimizationSettings : EditorWindow
    {
        ...
        {
            new setting("Shadows: Hard Only (recommended for low-end GPU)", false, true,
                () => { QualitySettings.shadows = ShadowQuality.HardOnly; }
            ),
            new setting("Shadow Projection: Close Fit", true, true,
                () => { QualitySettings.shadowProjection = ShadowProjection.CloseFit; }
            ),
            new setting("Shadow Distance: 1000", true, true,
                () => { QualitySettings.shadowDistance = 1000; }
            ),
            new setting("Shadow Cascades: 0", true, true,
                () => { QualitySettings.shadowCascades = 0; }
            ),
            new setting("Anisotropic Textures: Forced On", true, true,
                () => { QualitySettings.anisotropicFiltering = AnisotropicFiltering.ForceEnable; }
            ),
            new setting("vSync: off (recommended for low-end GPU)", false, true,
                () => { QualitySettings.vSyncCount = 0; }
            ),
            new setting("macOS Graphics API: OpenGLCore", true, false,
                () => {
            ...
            new setting("Run in Background", false, false,
            ...
            // ),
            new setting("Default is Fullscreen: true", true, false,
            ...
            new setting("Fullscreen Mode: Fullscreen Window", true, false,
#if UNITY_2018_1_OR_NEWER
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Apply Changes"))
            {
                ...
                Debug.Log(Misc.debugText + "Applied! By default, this popup will no longer appear, but you can access it by clicking HoloPlay/Optimization Settings Prompt");
                Close();
            }
            EditorGUILayout.EndHorizontal();
            ...
            {
                EditorPrefs.SetBool(editorPrefName + PlayerSettings.productName, true);
                Debug.Log(Misc.debugText + "Optimization popup hidden--" +
